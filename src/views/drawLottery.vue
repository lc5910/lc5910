<template>
  <div class="bg" ref="bg">
    <div class="content_1" v-show="avtiveCode==1">
      ...
      <a href="javascript:;" class="start" @click="start">
        <span></span>
      </a>
    </div>
  ...
  methods: {
    ...
    submit() {
      if (/^(13|14|15|18|19)\d{9}$/.test(this.mobile)) {
        ...
        this.$http
          .post("/pi/lottery/userBind", { mobile: this.mobile })
          .then(({ data }) => {
            ...
            if (data && data.code === 0) {
              ...
              if (
                data.data &&
                typeof data.data.lotteryStatus !== "undefined" &&
                data.data.lotteryStatus === "L0002"
              ) {
                gage.setStorage("prizeMobile", this.mobile, 1);
    ...
    alreadyGet() {
      ...
      MessageBox({
        ...
        message: "您的手机号已经抽过奖品，完成注册后即有客服人员联系您",
