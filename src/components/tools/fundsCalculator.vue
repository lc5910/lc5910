    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm">
      ...
      <el-form-item class="submitBox">
        ...
        <el-button
          ...
          :class="activeCal===2?'active':''"
        >反算本金</el-button>
        <el-button
          class="btn-submit"
          type="primary"
          size="medium"
          @click="dataFormSubmit(3)"
          :class="activeCal===3?'active':''"
        >反算涨跌</el-button>
        <el-button
          class="btn-submit"
          type="primary"
...
export default {
  data() {
    return {
      ...
      // 校验规则
      dataRule: {
        cost: [{ validator: this.moneyRule, trigger: "blur" }],
        productType: [{ validator: this.requiredRule, trigger: "blur" }],
        costServerCharge: [{ validator: this.moneyRule2, trigger: "blur" }],
        upRate: [{ validator: this.moneyRule, trigger: "blur" }],
        redeemServerCharge: [{ validator: this.moneyRule2, trigger: "blur" }],
        redeem: [{ validator: this.moneyRule, trigger: "blur" }]
      },
      productType: [
        {
          label: "指数C",
  ...
  methods: {
    ...
    dataFormSubmit(num) {
      this.activeCal = num;
      // 去除空格
      for (const val in this.dataForm) {
    ...
    ptChange(val) {
      ...
      this.dataForm.redeemServerCharge = this.productType[
        val
      ].redeemServerCharge;
      this.$nextTick(() => {
    ...
    calEarnings() {
      ...
      const cost = parseFloat(this.dataForm.cost);
      const costServerCharge = parseFloat(this.dataForm.costServerCharge);
      const upRate = parseFloat(this.dataForm.upRate);
      const redeemServerCharge = parseFloat(this.dataForm.redeemServerCharge);
      // 赎回金额
      this.dataForm.redeem = gage.toFixed2(
        cost *
          (1 - costServerCharge / 100) *
          (1 + upRate / 100) *
          (1 - redeemServerCharge / 100)
      );
      // 总手续费
      const sum = gage.toFixed2(
        (cost * costServerCharge) / 100 +
          (cost *
            (1 - costServerCharge / 100) *
            (1 + upRate / 100) *
            redeemServerCharge) /
      ...
      if (sum !== "") {
        this.dataForm.serverCharge =
          gage.toFixed2((cost * costServerCharge) / 100) +
          " + " +
          gage.toFixed2(
            (cost *
              (1 - costServerCharge / 100) *
      ...
      // 累计收益
      this.dataForm.earnings = gage.toFixed2(
        cost *
          (1 - costServerCharge / 100) *
          (1 + upRate / 100) *
          (1 - redeemServerCharge / 100) -
    ...
    calCost() {
      ...
      const upRate = parseFloat(this.dataForm.upRate);
      const redeemServerCharge = parseFloat(this.dataForm.redeemServerCharge);
      const redeem = parseFloat(this.dataForm.redeem);
      let cost =
        redeem /
        (1 - redeemServerCharge / 100) /
        (1 + upRate / 100) /
    ...
    // 保存图片
    downLoadImg() {
      const elem = document.querySelector(".fundsCalculator");
      ...
      html2canvas(elem, opts).then(function(canvas) {
        const newImageData = canvas.toDataURL("image/jpg");
        const blob = gage._base64Toblob(newImageData);
