(function (factory) {
  ...
  function veImage(opts) {
    ...
    //图片的宽高比
    var imgRate = this.image.width / this.image.height;

    this.dWidth = this.canvas.width;
    this.dHeight = this.dWidth / imgRate;
  ...
  veImagePrototype.crop = function (w, h, x, y) {
    ...
    context.drawImage(
      this.canvas,
      x, y, //开始剪切的 x 坐标
      canvas.width, canvas.height, //裁剪区域的大小
      0, 0, //在画布上放置图像的 x
      canvas.width, canvas.height //要使用的图像的宽度
    );
  ...
  veImagePrototype._draw = function (dx, dy, zoomWidth, zoomHeight) {
    ...
    if (this.dWidth * zoom >= 3000 || this.dWidth * zoom <= 600) {
      zoom = 1; //限制缩放大小
    }
    this.dWidth *= zoom; //宽度缩放
    this.dHeight *= zoom; //高度缩放

    // document.title=this.dWidth;  //log
    //处理IOS的BUG
    ...
    this.context.drawImage(
      this.image,
      0, 0,
      this.image.width, this.image.height, -this.dWidth / 2, -this.dHeight / 2,
